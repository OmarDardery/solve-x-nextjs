...
import { BookOpen, FileText, TrendingUp, ExternalLink, Loader2 } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { Badge } from "@/components/ui/Badge";
import { applicationApi, opportunityApi, type Application, type Opportunity } from "@/lib/api";
  ...
  const fetchData = async () => {
    try {
      const [appsData, oppsData] = await Promise.all([
        ...
        opportunityApi.getAll({ status: "open" }).catch(() => []),
      ]);
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="card flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <Button as={Link} href="/opportunities" variant="secondary" className="text-sm sm:text-base">
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: "var(--card-border)" }}>
                    <div className="text-xs text-muted">
                      {opportunity.tags?.length || 0} tags
                    </div>
